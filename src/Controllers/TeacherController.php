...
class TeacherController {
    ...
    public function listStudents() {
        ...
        if ($this->supabaseAvailable()) {
            ...
            $filters = [];
            if ($role && $role !== 'all') { $filters['role'] = 'eq.' . $role; }
            if ($q !== '') { $filters['or'] = sprintf('(name.ilike.*%s*,email.ilike.*%s*)', urlencode($q), urlencode($q)); }
            ...
            if (($groupId && $groupId > 0) || in_array($estado, ['with_group','without_group'], true)) {
                ...
                if ($groupId && $groupId > 0) { $gmEndpoint .= '&group_id=eq.' . $groupId; }
                $gmRes = supabaseRequest('GET', $gmEndpoint);
                $ids = [];
                if ($gmRes['code'] >= 200 && is_array($gmRes['data'])) {
                    foreach ($gmRes['data'] as $row) { if (!empty($row['student_id'])) { $ids[$row['student_id']] = true; } }
                }
                ...
                if ($idList !== '') {
                    ...
                    if (!$role || $role === 'all') { $filters['role'] = 'eq.student'; }
                } else {
                    if ($estado === 'with_group') { echo json_encode(['data' => []]); return; }
                    if (!$role || $role === 'all') { $filters['role'] = 'eq.student'; }
                }
            }
        ...
        $list = array_values(array_filter($users, function($u) use ($role,$q){
            $r = strtolower($u['role'] ?? '');
            if ($role && $role !== 'all' && $r !== $role) return false;
            if ($q !== '' && stripos(($u['name'] ?? '') . ' ' . ($u['email'] ?? ''), $q) === false) return false;
            return true;
        }));
    ...
    public function analytics() {
        header('Content-Type: application/json');
        $students = 0; $teachers = 0; $sessions = 0; $exercises = 0;
        ...
        if ($this->supabaseAvailable()) {
            ...
            if ($resUsers['code'] >= 200 && is_array($resUsers['data'])) {
                foreach ($resUsers['data'] as $u) {
                    $r = $u['role'] ?? '';
                    if ($r === 'student') $students++; elseif ($r === 'teacher') $teachers++;
                }
            }
            $resSess = supabaseRequest('GET', 'lab_sessions?select=id&limit=100');
        if ($resSess['code'] >= 200 && is_array($resSess['data'])) { $sessions = count($resSess['data']); }
        } else {
            $users = $this->readJson(__DIR__ . '/../../storage/users.json');
            foreach ($users as $u) {
                $r = $u['role'] ?? '';
    ...
    public function addCommunication() {
        ...
        if ($msg === '') { http_response_code(400); echo json_encode(['error' => 'empty_message']); return; }
    ...
    public function listGroups() {
        ...
        if ($this->supabaseAvailable()) {
            ...
            $data = is_array($res['data']) ? $res['data'] : [];
            echo json_encode(['data' => $data]); return;
        }
        echo json_encode(['data' => []]);
    }
    ...
    public function performComputerActions() {
        ...
        $ids = isset($input['computer_ids']) && is_array($input['computer_ids']) ? array_map('intval', $input['computer_ids']) : [];
        if (!$action || empty($ids)) { http_response_code(400); echo json_encode(['error' => 'action_type y computer_ids requeridos']); return; }
        $allowed = ['power_on','power_off','restart','lock','unlock'];
        if (!in_array($action, $allowed, true)) { http_response_code(400); echo json_encode(['error' => 'action_type inválido']); return; }
        $created = 0; $errors = [];
        foreach ($ids as $cid) {
            if ($cid <= 0) { continue; }
            $payload = [
    ...
    public function listTeacherCourses() {
        ...
        if (!$this->supabaseAvailable()) {
            ...
            $data = array_map(function($c){ return ['id' => $c['id'], 'title' => $c['title']]; }, $filtered);
            echo json_encode(['data' => $data]);
            return;
        }
    ...
    public function listExercises() {
        ...
        if ($courseId <= 0) { echo json_encode(['data' => []]); return; }
        ...
        if (!$this->supabaseAvailable()) {
            ...
            $modIds = array_column(array_values(array_filter($mods, function($m) use ($courseId){ return (int)($m['course_id'] ?? 0) === $courseId; })), 'id');
            if (empty($modIds)) { echo json_encode(['data' => []]); return; }
            $out = array_values(array_filter($exercises, function($e) use ($modIds){ return in_array($e['module_id'] ?? 0, $modIds, true); }));
            echo json_encode(['data' => array_map(function($e){ return ['id'=>$e['id'],'title'=>$e['title'],'module_id'=>$e['module_id']]; }, $out)]);
            return;
        }
        // Camino normal con Supabase
        ...
        $modIds = array_map(function($m){ return (int)($m['id'] ?? 0); }, $mods);
        $modIds = array_values(array_filter($modIds, function($v){ return $v > 0; }));
        if (empty($modIds)) { echo json_encode(['data' => []]); return; }
    ...
    public function listGrades() {
        ...
        if (!$this->supabaseAvailable()) {
            ...
            if ($exerciseId > 0) {
                $data = array_values(array_filter($grades, function($g) use ($exerciseId){ return (int)($g['exercise_id'] ?? 0) === $exerciseId; }));
                echo json_encode(['data' => $data]); return;
            }
            if ($courseId > 0) {
                $data = array_values(array_filter($grades, function($g) use ($courseId){ return (int)($g['course_id'] ?? 0) === $courseId; }));
                echo json_encode(['data' => $data]); return;
            }
        ...
        if ($exerciseId > 0) {
            ...
            $endpoint = 'submissions?select=id,user_id,exercise_id,score,grade,updated_at&exercise_id=eq.' . $exerciseId;
            $res = supabaseRequest('GET', $endpoint);
            $data = is_array($res['data'] ?? null) ? $res['data'] : [];
            echo json_encode(['data' => $data]); return;
        }
        if ($courseId > 0) {
            ...
            $endpoint = 'grades?select=id,user_id,course_id,module_id,final_grade,updated_at&course_id=eq.' . $courseId;
            $res = supabaseRequest('GET', $endpoint);
            $data = is_array($res['data'] ?? null) ? $res['data'] : [];
            echo json_encode(['data' => $data]); return;
        }
    ...
    public function createGrade() {
        ...
        if (!$userId || !$courseId || ($final === null)) { echo json_encode(['error' => 'missing_fields']); return; }
        ...
        if (!$this->supabaseAvailable()) {
            ...
            if (!is_array($grades)) { $grades = []; }
            $nextId = 1;
            foreach ($grades as $g) { $gid = (int)($g['id'] ?? 0); if ($gid >= $nextId) $nextId = $gid + 1; }
            $item = [
                'id' => $nextId,
                'user_id' => (int)$userId,
            ...
            if ($moduleId !== null) { $item['module_id'] = (int)$moduleId; }
            if ($exerciseId !== null) { $item['exercise_id'] = (int)$exerciseId; }
            $grades[] = $item;
    ...
    public function updateGrade() {
        ...
        if ($id <= 0) { http_response_code(422); echo json_encode(['error' => 'id_required']); return; }
            ...
            for ($i = 0; $i < count($grades); $i++) {
                if ((int)($grades[$i]['id'] ?? 0) === $id) {
                    foreach (['user_id','course_id','module_id','final_grade','exercise_id'] as $k) {
                        if (array_key_exists($k, $payload)) { $grades[$i][$k] = $payload[$k]; }
                    }
                    $grades[$i]['updated_at'] = date('c');
                    $found = true; break;
                }
            }
            if (!$found) { http_response_code(404); echo json_encode(['error' => 'not_found']); return; }
            $this->writeJson($gradesPath, $grades);
        ...
        // Camino normal con Supabase
        $data = [];
        foreach (['user_id','course_id','module_id','final_grade'] as $k) { if (array_key_exists($k, $payload)) { $data[$k] = $payload[$k]; } }
    ...
    public function deleteGrade() {
        ...
        if (!$this->supabaseAvailable()) {
            ...
            $grades = $this->readJson($gradesPath);
            $before = count($grades);
            $grades = array_values(array_filter($grades, function($g) use ($id){ return (int)($g['id'] ?? 0) !== $id; }));
            if (count($grades) === $before) { http_response_code(404); echo json_encode(['error' => 'not_found']); return; }
            $this->writeJson($gradesPath, $grades);
