...
class StartCodesController {
    ...
    private function loadAll() {
        ...
        } catch (\Throwable $e) { \App\Core\Logger::getInstance()->error('start_codes load error', ['msg' => $e->getMessage()]); return []; }
    }
    ...
    public function saveBulk() {
        ...
        $map = [];
        foreach ($existing as $it) { $map[$it['code']] = $it; }
        foreach ($codes as $it) {
            if (!isset($it['code'])) continue;
            $code = strtoupper(trim($it['code']));
            if (!preg_match('/^[A-Z0-9_-]{3,64}$/', $code)) { continue; }
            $role = strtolower(trim($it['role'] ?? 'teacher'));
            if (!in_array($role, ['teacher','student','admin'])) { $role = 'teacher'; }
            $map[$code] = [
                'role' => $role,
                'code' => $code,
    ...
    public function validate() {
        ...
        if ($code === '') { return $this->json(['valid' => false, 'reason' => 'missing'], 400); }
        $items = $this->loadAll();
        $found = null;
        foreach ($items as $it) { if (($it['code'] ?? '') === $code) { $found = $it; break; } }
        if (!$found) { return $this->json(['valid' => false, 'reason' => 'not_found']); }
        if (!empty($found['used'])) { return $this->json(['valid' => false, 'reason' => 'used']); }
        if (!empty($found['expires_at'])) {
            $t = strtotime($found['expires_at']);
            if ($t && time() > $t) { return $this->json(['valid' => false, 'reason' => 'expired']); }
        }
    ...
    public function useCode() {
        ...
        $code = preg_replace('/[^A-Z0-9_-]/', '', $code);
        if ($code === '') { return $this->json(['ok' => false, 'error' => 'missing_code'], 400); }
        $items = $this->loadAll();
        $updated = false;
        foreach ($items as &$it) {
            if (($it['code'] ?? '') === $code) { $it['used'] = true; $updated = true; break; }
        }
        unset($it);
        if (!$updated) { return $this->json(['ok' => false, 'error' => 'not_found'], 404); }
        $this->saveAll($items);
        $this->json(['ok' => true]);
    }
