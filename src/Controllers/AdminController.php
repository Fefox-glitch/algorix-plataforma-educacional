...
class AdminController
{
    ...
    public function listLabs()
    {
        $offline = false;
// offline eliminado
        $endpoint = 'computer_labs?select=id,name,location,capacity,is_active,created_by,updated_by,' .
            'created_at,updated_at&deleted_at=is.null';
        $res = supabaseRequest('GET', $endpoint);
        $code = (int)$res['code'];
        $data = is_array($res['data'] ?? null) ? $res['data'] : [];
        $error = $res['error'] ?? null;
        if ($code >= 400) {
            return $this->json($code, null, $error ?: 'upstream_error');
        }
...
// Validación estricta
        ...
        if ($name === '' || $this->strlen_utf8($name) > 100) {
            return $this->json(400, null, 'name inválido');
        }
        if ($location !== '' && $this->strlen_utf8($location) > 150) {
            return $this->json(400, null, 'location inválido');
        }
        if ($capacity < 0 || $capacity > 10000) {
            return $this->json(400, null, 'capacity inválido');
        }
        $createdBy = (
            isset($_SESSION['user']['id']) &&
            is_string($_SESSION['user']['id']) &&
            preg_match('/^[0-9a-fA-F-]{36}$/', $_SESSION['user']['id'])
...
// offline eliminado
    ...
    public function updateLab()
    {
        ...
        if ($id <= 0) {
            return $this->json(400, null, 'id inválido');
        }
        $fields = [];
...
// offline eliminado
        $endpoint = 'computers?select=id,name,lab_id,ip_address,mac_address,status,last_seen,last_boot,' .
            'created_by,updated_by,updated_at&deleted_at=is.null';
...
// Validación estricta
        ...
        if ($labId <= 0) {
            return $this->json(400, null, 'lab_id inválido');
        }
        if ($ip !== '' && !filter_var($ip, FILTER_VALIDATE_IP)) {
            return $this->json(400, null, 'ip_address inválido');
        }
        if ($mac !== '' && !preg_match('/^([0-9A-Fa-f]{2}:){5}[0-9A-Fa-f]{2}$/', $mac)) {
            return $this->json(400, null, 'mac_address inválido');
        }
        if (!in_array($status, $allowedStatus, true)) {
            return $this->json(400, null, 'status inválido');
        }
        $createdBy = (
            isset($_SESSION['user']['id']) &&
            is_string($_SESSION['user']['id']) &&
...
// offline eliminado
    ...
    public function listGroups()
    {
        $offline = false;
// offline eliminado
        $endpoint = 'student_groups?select=id,name,teacher_id,lab_id,is_active,created_by,updated_by,' .
            'created_at,updated_at&deleted_at=is.null';
        $res = supabaseRequest('GET', $endpoint);
        $code = (int)$res['code'];
        $data = is_array($res['data'] ?? null) ? $res['data'] : [];
...
// Validación estricta
        ...
        if ($teacherIdRaw === '' || !preg_match('/^[0-9a-fA-F-]{36}$/', $teacherIdRaw)) {
            return $this->json(400, null, 'teacher_id inválido (UUID requerido)');
        }
        if ($labId <= 0) {
            return $this->json(400, null, 'lab_id inválido');
        }
...
// offline eliminado
    ...
    public function listActions()
    {
        $offline = false;
// offline eliminado
        $endpoint = 'computer_actions?select=id,computer_id,action_type,performed_by,status,result,performed_at' .
            '&order=performed_at.desc&limit=100';
        $res = supabaseRequest('GET', $endpoint);
        $code = (int)$res['code'];
        $data = is_array($res['data'] ?? null) ? $res['data'] : [];
        $error = $res['error'] ?? null;
...
// offline eliminado
    ...
    public function getAIConfig()
    {
        ...
        if (!$isAdmin) {
            return $this->json(403, [], 'forbidden');
        }
        $path = __DIR__ . '/../../storage/ai_config.json';
        $defaults = [
            'AI_ENDPOINT' => isset($_ENV['AI_ENDPOINT'])
                ? $_ENV['AI_ENDPOINT']
                : 'https://oi-server.onrender.com/chat/completions',
            'AI_MODEL' => isset($_ENV['AI_MODEL'])
                ? $_ENV['AI_MODEL']
                : 'openrouter/claude-sonnet-4',
            'AI_TOKEN' => isset($_ENV['AI_TOKEN'])
                ? $_ENV['AI_TOKEN']
                : '',
            'AI_CUSTOMER_ID' => isset($_ENV['AI_CUSTOMER_ID'])
                ? $_ENV['AI_CUSTOMER_ID']
                : '',
            'OFFLINE_MODE' => (isset($_ENV['OFFLINE_MODE']) && $_ENV['OFFLINE_MODE'] === 'true')
                ? 'true'
                : 'false'
        ];
        $cfg = $defaults;
    ...
    public function saveAIConfig()
    {
        ...
        $data = json_decode($raw, true) ?: [];
        $cfg = [
            'AI_ENDPOINT' => isset($data['AI_ENDPOINT'])
                ? trim($data['AI_ENDPOINT'])
                : '',
            'AI_MODEL' => isset($data['AI_MODEL'])
                ? trim($data['AI_MODEL'])
                : '',
            'AI_TOKEN' => isset($data['AI_TOKEN'])
                ? trim($data['AI_TOKEN'])
                : '',
            'AI_CUSTOMER_ID' => isset($data['AI_CUSTOMER_ID'])
                ? trim($data['AI_CUSTOMER_ID'])
                : '',
            'OFFLINE_MODE' => (
                isset($data['OFFLINE_MODE']) &&
                ($data['OFFLINE_MODE'] === true || $data['OFFLINE_MODE'] === 'true')
            )
                ? 'true'
                : 'false'
        ];
        ...
        if ($ok === false) {
            return $this->json(500, [], 'write_error');
        }
